		...
		<div class="logowrapper col-md-12">
			...
			<div class="col-md-7 col-sm-12 col-xs-12" id="login">	
				...
				<form class="form-inline" action='login_check.php' method='post'>
					...
					<button type='submit' name='login_btn' value='Login' class="btn btn-default login_button">Log in</button></br>
					<a href='reg_index.php' class="link link-info link-lg" data-target="#myModal">
						<button class="btn btn_default registrate_button">Register!</button>
					</a>
				</form>	
			</div>
	...
	<div class="row">
		...
		<div class="field col-md-12">
			...
			<div class="teaser col-md-6 col-xs-12">
				<p><span>Fifa World Cup is here! </span></br> 
					Don't miss the opportunity to bet against your friends.</br>
...
function gamesToday(){
	...
	if (!$has_game){
		echo "<p class='no_game'>Sorry, no games today.</p>";
	}
...
function gamesTomorrow(){
	...
	while($row = mysqli_fetch_assoc($result)){
		...
		if($datetime == date('Ymd', strtotime($game_start))){
			...
			echo "Sorry, no games tomorrow.";
		}								
	}
	if (!$has_game){
		echo "<p class='no_game'>Sorry, no games tomorrow.</p>";
	}
}
